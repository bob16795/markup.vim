...
function! s:MarkupRefreshSyntax(force)
    if &filetype =~ 'markup' && line('$') > 1
...
augroup Mu
    ...
    autocmd! * <buffer>
    autocmd BufWinEnter <buffer> call s:MarkupRefreshSyntax(1)
    autocmd BufWritePost <buffer> call s:MarkupRefreshSyntax(0)
    autocmd InsertEnter,InsertLeave <buffer> call s:MarkupRefreshSyntax(0)
    autocmd CursorHold,CursorHoldI <buffer> call s:MarkupRefreshSyntax(0)
augroup END
